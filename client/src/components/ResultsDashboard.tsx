...
export function ResultsDashboard({
  ...
  const avgScore =
    candidates.length > 0
      ? Math.round(
          candidates.reduce((sum, c) => sum + c.normalizedScore, 0) /
            candidates.length,
        )
      : 0;

  const topCandidates = candidates.filter(
    (c) => c.normalizedScore >= 70,
  ).length;
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={item}
        className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card p-4 rounded-xl border shadow-sm"
      >
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          {(["rank", "normalizedScore", "candidateName"] as SortField[]).map(
            (field) => (
              <Button
                key={field}
      ...
      <div className="space-y-4">
        {candidates.length === 0 ? (
          <motion.div
            variants={item}
            className="text-center py-20 px-6 bg-muted/30 rounded-xl border border-dashed"
          >
